...
# Type of simulation to run (the functions to create the discretization are in the space.cpp file):
   # simulation=
                 # HH_Model_Only: only one HH element. This simulation is only available with the sequential version of the explicit scheme
...
case $3 in
    "example_template")
	# dtf implicit=100 explicit=0.8
	echo "Explicit simulations for one axon"
	OPT="g Tt 0.025 IRE 20 NRE 1 dtf ${dtf} xspan 0.02"
