...
# Type of simulation to run (the functions to create the discretizations are in the space.cpp file):
   # simulation=
                 ...
                 # CT_cable:  paassive structure without branching
...
export simulation=Only_HH_Model; 
...
case $3 in
    "example_1")
	echo "Potential of one ion channel"
	#OPT=""	
...
echo "time ${COMM} ${S_TYPE} ${OPT} out ${PATH}/${SOLVER}_simulation.dat log ${PATH}/${SOLVER}_simulation.log"
time ./${COMM} ${S_TYPE} ${OPT} out ${PATH}/${SOLVER}_simulation.dat log ${PATH}/${SOLVER}_simulation.log 
